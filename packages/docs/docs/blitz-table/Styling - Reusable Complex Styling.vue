...
const propsToPass = computed(() => {
  return {
    ...
    schemaColumns: props.schemaColumns || [...schemaColumns],
    schemaGrid: props.schemaGrid || [...schemaColumns],
    title: props.title || 'My Styled Table',
    actionButtons: [],
  }
})
</script>
...
<template>
  <div>
    ...
    <div :class="`title-section-above-table`">
      <h6>{{ propsToPass.title }}</h6>
      <!-- prettier-ignore -->
      <svg :class="['_grid-rows-toggle', { _active: grid === true }]" width="24" height="24" viewBox="0 0 24 24" @click="() => (grid = true)">
      ...
      <!-- prettier-ignore -->
      <svg :class="['_grid-rows-toggle', { _active: grid === false }]" width="24" height="24" viewBox="0 0 24 24" @click="() => (grid = false)">
